...
struct Application
{
	...
	uint32_t ProjectorUBO;
	uint32_t ProjectorTexture;
	...
	void Initialize()
	{
		...
		int projectorBlockIndex = glGetUniformBlockIndex(program, "ProjectorMatrices");
		glUniformBlockBinding(program, projectorBlockIndex, ProjectorBindingIndex);

		ProjectorTexture = Texture::LoadTexture("Batman_logo.png");
	...
	void RenderOffscreen()
	{
		...
		projectorMatrices[0] = 
			glm::lookAt(positionProjector, glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f));
	
		float projectorAR = 1.0f;
		projectorMatrices[1] =
			glm::perspective(glm::radians(45.f), projectorAR, 0.1f, 1000.f);
		...
		viewMatrix = glm::lookAt(positionCamera, glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 1.f, 0.f));// *rotateCam;
		...
		positionCamera = glm::vec3(cameraTransformMatrix[3]);		
		...
		positionCamera = -glm::vec3(viewMatrix[3]);
		positionCamera = minor * positionCamera;

		glUniform3fv(camPosLocation, 1, &positionCamera.x);
...
int main(int argc, const char* argv[])
{
	...
	window = glfwCreateWindow(960, 720, "OBJ Viewer PBR basic", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
